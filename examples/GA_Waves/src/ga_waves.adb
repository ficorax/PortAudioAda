...
with GA_Waves_Callbacks; use GA_Waves_Callbacks;
with GA_Waves_Globals; use GA_Waves_Globals;
with GA_Waves_Package; use GA_Waves_Package;
...
begin
   ...
   Gtk_New (combo);
   Attach (table, combo, 1, 2, 2, 3);
   Append_Text (combo, "Sine");
   Append_Text (combo, "Square");
   Append_Text (combo, "Sawtooth");
   Append_Text (combo, "Triangle");
   Append_Text (combo, "White Noise");
   Append_Text (combo, "Pink Noise");
   Append_Text (combo, "Brown Noise");
   Set_Active (combo, 0);
   CombHand.Connect
     (combo,
      "changed",
      CombHand.To_Marshaller (Wave_Type_Callback'Access));
   options (EO_Wave_Type).optionWidget := combo;
   options (EO_Wave_Type).optionUse    := OU_Before;
   ...
   options (EO_Start).optionWidget := button;
   options (EO_Start).optionUse    := OU_Before;
